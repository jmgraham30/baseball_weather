...
Based on the test error metrics, we conclude that this model is mediocre with regard to its predictive accuracy although its qualitative results are consistent with those of our linear regression analysis.  
...
The results of the tuned decision tree model for game duration are shown in @fig-duration_vip and @fig-duration_preds. We observe that the most important predictors for the model are the date and the number of runs scored by each team. The model predictions are plotted against the observed values in @fig-duration_preds. We observe that the model does a reasonable job of predicting the game duration except when the games are particularly long.
